library(shiny)
library(sets)   # lógica difusa

sets::sets_options("universe", seq(1, 100, 1))
...
ui <- fluidPage(
    ...
    helpText("Informe os valores nas barras de deslizamento"),
    
    fluidRow(
        column(4,
               sliderInput("exatas", "Gosto por Exatas", min = 5, max = 90, step = 10, value = 40)
        ),
        column(4,
               sliderInput("interpessoal", "Relacionamento Interpessoal", min = 30, max = 85, step = 5, value = 50)
        ),
        column(4,
               sliderInput("codigo", "Gosto por Programação", min = 10, max = 95, step = 5, value = 40)
        )
    ),
    
    fluidRow(
        column(4,
               sliderInput("lider", "Perfil de Liderança", min = 30, max = 95, step = 5, value = 50)
        ),
        column(4,
               sliderInput("estudar", "Gosta de Estudar", min = 20, max = 90, step = 10, value = 40)
        ),
        column(4,
               sliderInput("comunicacao", "Habilidade de Comunicação", min = 40, max = 75, step = 5, value = 50)
        )
    ),
    ...
    fluidRow(
        column(6,
               h1("Sistema:"),
               plotOutput("graficosistema")
        ),
        column(6,
               actionButton("processar", "Processar"),
               helpText("A linha vermelha mostra a sua aderência a profissão"),
...
server <- function(input, output) {
    ...
    observeEvent(input$processar, {
        inferencia <- fuzzy_inference(sistema, list(Gosta_exatas = input$exatas,
                                                    Rel_interpess = input$interpessoal,
                                                    Escre_codigo = input$codigo,
                                                    Pefil_lider = input$lider,
                                                    Gosta_Estudar = input$estudar,
                                                    Habilidade_comunicao = input$comunicacao))
